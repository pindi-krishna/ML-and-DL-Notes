...
1.  As K increases, loss always decreases. Therefore, we consider elbow point. 
1.  Difficult to find this point most of the times. If failed to find the optimal point by measuring other metrics at each value of K.
...
1.  This algorithm highly depends on the initialization. 
...
1.  The elbow method used to select the number of clusters doesn’t work well as the error function decreases for all Ks.
...
1. Pseudo code of the algorithm can be found below. 
...
A GM is a function composed of several Gaussians, each identified by $K \in {1,…, K}$, where K is the number of clusters. Each Gaussian K in the mixture is comprised of following parameters:

1.  A mean $\mu$ that defines its center.
1.  A covariance $\sum$ that defines its width.
1.  A mixing probability that defines how big or small or big the Gaussian function will be.
